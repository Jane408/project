        ...
        if (!storedPromotion) {
          let randomDiscount =
            validPromotions.length > 0
              ? validPromotions[
                  Math.floor(Math.random() * validPromotions.length)
                ].discountPercentage
              : "";
          localStorage.setItem(
            `promotion_${product.id}`,
            JSON.stringify(randomDiscount)
          );
          product.discountPercentage = randomDiscount;
        } else {
      ...
      productListDiv.innerHTML += `
        <div class="product__item">
          <div class="product__banner">
            <a href="#" class="product__images" onclick="goToProductDetails(${
              product.id
            })">
              <img
